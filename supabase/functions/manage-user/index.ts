  ...
  if (!profile || !profile.ativo || (profile.role !== "dono" && profile.role !== "admin")) {
    return {
      ...
      response: jsonResponse(403, {
        success: false,
        code: "forbidden",
        error: "Only active owners or admins can manage users.",
      }),
    }
  }
...
async function handleCreateUser(
  ...
  if (!isValidRole(payload.role)) {
    return jsonResponse(400, {
      success: false,
      code: "invalid_role",
      error: "role must be dono, admin or corretor.",
    })
  }
  ...
  if (requesterRole === "admin" && payload.role !== "corretor") {
    return jsonResponse(403, {
      ...
      error: "Admins can only create broker accounts.",
    })
  }
  ...
  if (payload.role === "dono" && requesterRole !== "dono") {
    return jsonResponse(403, {
      ...
      error: "Only owners can create another owner.",
    })
  }
  ...
  if (payload.role === "admin" && requesterRole !== "dono") {
    return jsonResponse(403, {
      ...
      error: "Only owners can create administrator accounts.",
    })
  }
...
async function handleDeleteUser(
  ...
  if (targetProfileError) {
    console.error("Failed to inspect target profile before deletion", targetProfileError)
    return jsonResponse(500, {
      success: false,
      code: "target_profile_lookup_failed",
      error: "Could not validate the target profile before deletion.",
    })
  }
...
Deno.serve(async (request) => {
  ...
  const adminResult = await requireManager(request)
  ...
  if (payload.action === "createUser") {
    return handleCreateUser(adminResult.serviceClient, adminResult.requesterRole, payload)
  }

  if (payload.action === "deleteUser") {
    return handleDeleteUser(
      adminResult.serviceClient,
      adminResult.requesterId,
      adminResult.requesterRole,
      payload
    )
  }
