...
export default function GlobalLeadSearch({
  ...
  variant?: "default" | "sidebar" | "sidebar-full"
}) {
    ...
    <>
      {variant === "sidebar" ? (
        ...
        <Button
          type="button"
          variant="ghost"
          className="h-13 w-full justify-start gap-3 rounded-[1.1rem] px-4 py-3 text-left text-sidebar-foreground"
          onClick={() => setOpen(true)}
        >
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-sidebar-border/70 bg-sidebar text-sidebar-foreground">
            <Search className="h-4 w-4" />
          </div>
          <div className="min-w-0 flex-1 space-y-0.5">
            <p className="truncate text-sm font-medium">Pesquisar lead</p>
            <p className="truncate text-xs text-sidebar-foreground/65">Nome, telefone ou e-mail</p>
          </div>
          <span className="hidden rounded-full border border-sidebar-border/70 bg-sidebar px-2.5 py-1 text-[11px] uppercase tracking-[0.14em] text-sidebar-foreground/65 xl:inline-flex">
            Ctrl+K
          </span>
        </Button>
      ) : (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1 space-y-1.5">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {isManualLead(lead) ? (
                            <Badge className="crm-badge-brand min-h-6 rounded-full px-2.5 text-[11px] font-medium">
                              Manual
                            </Badge>
                          ) : null}
                          <Badge
