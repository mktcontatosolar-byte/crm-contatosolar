...
export default function ManualLeadDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        showCloseButton
        className="max-h-[90vh] max-w-6xl gap-0 overflow-hidden rounded-[2rem] p-0"
      >
        <DialogHeader className="shrink-0 border-b border-border/60 bg-card/95 px-6 pt-6 pb-5 backdrop-blur">
          <DialogTitle>Adicionar lead manual</DialogTitle>
          <DialogDescription>
            Esse lead entra direto no CRM, já fica atribuído a quem criou e aparece marcado como manual.
          </DialogDescription>
        </DialogHeader>
        ...
        <form className="flex min-h-0 flex-1 flex-col" onSubmit={handleSubmit}>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      value={form.conta}
                      onChange={(event) =>
                        updateField("conta", event.target.value as ManualLeadFormState["conta"])
                      }
                      disabled={isSubmitting}
                    >
          ...
          <DialogFooter className="shrink-0 border-t border-border/60 bg-card/95 px-6 py-5 backdrop-blur">
            <Button
              type="button"
              variant="outline"
